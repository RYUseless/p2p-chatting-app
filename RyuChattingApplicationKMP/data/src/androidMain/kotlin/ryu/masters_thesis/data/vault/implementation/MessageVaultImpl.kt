...
class MessageVaultImpl(
    ...
    private val db: AppDatabase by lazy {
        Room.databaseBuilder<AppDatabase>(
            ...
            name    = context.getDatabasePath("vault.db").absolutePath,
        )
    ...
    override suspend fun storeRoomMetadata(
        ...
        val encName      = cipher.encrypt(nameBytes, hashedRoomId).getOrThrow()
        val encPass      = cipher.encrypt(passBytes, hashedRoomId).getOrThrow()
    ...
    override suspend fun getRooms(): Result<List<RoomSummary>> = runCatching {
        db.roomMetadataDao().getAll().mapNotNull { metadata ->
            ...
            val role = RoomRole.valueOf(metadata.role)
    ...
    //new, snad to fixne issue s mazanim roomky
