...
// entry point pro HomeContent → reálný UI setup
        ...
        LaunchedEffect(Unit) {
            screenModel.oneTimeEvents.collect { event ->
                when (event) {
                    ...
                    is HomeOneTimeEvent.NavigateToReconnect ->
                        navigator.push(ReconnectScreen(roomName = event.roomName, peerAddress = event.peerAddress))
                    is HomeOneTimeEvent.NavigateToCreate    ->
                        navigator.push(CreateScreen(roomName = event.roomName))
                    is HomeOneTimeEvent.ShowError           -> Unit
                }
            }
        }
